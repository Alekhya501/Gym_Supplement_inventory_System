...
SET stock_quantity = 0;
   ...
   # TRIGGERS (increase stock on sale
...
VALUES (1, 10, 1500.00, '2024-03-25');
INSERT INTO sales (product_id, quantity, selling_price, sale_date) 
VALUES (1, 5, 2000.00, '2024-03-25');

	#updating product price for new purchase price
...
FOR EACH ROW
    ...
    #View Purchase History of a Product
SELECT p.name, pu.quantity, pu.purchase_price, pu.purchase_date 
FROM purchases pu 
JOIN products p ON pu.product_id = p.id 
WHERE p.id = 1;
...
MODIFY COLUMN category ENUM('protein','vitamins','creatine','pre-workouts','post_workouts','energy_drinks') NOT NULL;
ALTER TABLE products 
